...
void findTopPlayer(Player* all_players, int player_num) {
    ...
    for (int i = 1; i < 1+player_num; i++) {
        if (!all_players[i].failed) {
            ...
            all_players[i].score = all_players[i].score + all_players[i].PlayerAttack + all_players[i].PlayerLife + all_players[i].PlayerMaxLife; // calculate the score of the player
